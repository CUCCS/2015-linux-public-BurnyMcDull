...
per20=$(awk 'BEGIN{printf "%.2f",'$age20*100/$total'}')
per2030=$(awk 'BEGIN{printf "%.2f",'$age2030*100/$total'}')
per30=$(awk 'BEGIN{printf "%.2f",'$age30*100/$total'}')
echo '20岁以下的人数和比例分别为：'$age20' '$per20'%'
echo '20到30岁的人数和比例分别为：'$age2030' '$per2030'%'
echo '30岁以下的人数和比例分别为：'$age30' '$per30'%'
echo ''
echo $oldestname'是最年长的球员,今年'$oldest'岁'
echo $youngestname'是最年轻的球员,今年'$young'岁'
echo ''
